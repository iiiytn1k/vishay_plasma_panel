...
unsigned char font [256][5] = {                                            
    ...
    0x00, 0x00, 0x00, 0x00, 0x00 ,   // Ґ 0xA2 162
    ...
    0x4A, 0x95, 0xA5, 0xA9, 0x52 ,   // І 0xA7 167
    0x7C, 0x55, 0x54, 0x45, 0x44 ,   // ® 0xA8 168
    0x00, 0x18, 0x24, 0x24, 0x00 ,   // © 0xA9 169
    0x3E, 0x49, 0x49, 0x41, 0x22 ,   // ™ 0xAA 170
    0x08, 0x14, 0x2A, 0x14, 0x22 ,   // Ђ 0xAB 171
    0x04, 0x04, 0x04, 0x04, 0x0C ,   // ђ 0xAC 172
    0x00, 0x08, 0x08, 0x08, 0x00 ,   // ≠ 0xAD 173
    0x00, 0x00, 0x00, 0x00, 0x00 ,   // Ѓ 0xAE 174
    0x00, 0x45, 0x7C, 0x45, 0x00 ,   // ѓ 0xAF 175
    ...
    0x00, 0x78, 0x08, 0x0C, 0x00 ,   // і 0xB4 180
    0x00, 0xFC, 0x20, 0x3C, 0x20 ,   // µ 0xB5 181
    0x0C, 0x1E, 0xFE, 0x02, 0xFE ,   // ґ 0xB6 182
    0x00, 0x18, 0x18, 0x00, 0x00 ,   // Ј 0xB7 183
    0x39, 0x54, 0x54, 0x49, 0x00 ,   // Є 0xB8 184
    0x78, 0x10, 0x20, 0x7B, 0x03 ,   // є 0xB9 185
    0x38, 0x54, 0x54, 0x44, 0x00 ,   // Ї 0xBA 186
    0x22, 0x14, 0x2A, 0x14, 0x08 ,   // ї 0xBB 187
    0x00, 0x00, 0x00, 0x00, 0x00 ,   // Љ 0xBC 188
    0x00, 0x00, 0x00, 0x00, 0x00 ,   // љ 0xBD 189
    0x00, 0x00, 0x00, 0x00, 0x00 ,   // Њ 0xBE 190
    0x00, 0x54, 0x70, 0x44, 0x00 ,   // њ 0xBF 191
    0x7C, 0x12, 0x11, 0x12, 0x7C ,   // ј 0xC0 192
    0x7F, 0x49, 0x49, 0x49, 0x31 ,   // Ѕ 0xC1 193
    ...
    0x40, 0x3E, 0x01, 0x01, 0x7F ,   // Ћ 0xCB 203
    0x7F, 0x02, 0x0C, 0x02, 0x7F ,   // ћ 0xCC 204
    0x7F, 0x08, 0x08, 0x08, 0x7F ,   // Ќ 0xCD 205
    0x3E, 0x41, 0x41, 0x41, 0x3E ,   // ќ 0xCE 206
    0x7F, 0x01, 0x01, 0x01, 0x7F ,   // ѕ 0xCF 207
    ...
    0x7F, 0x40, 0x7E, 0x40, 0x7F ,   // Ў 0xD8 216
    0x3F, 0x20, 0x3F, 0x20, 0x7F ,   // ў 0xD9 217
    0x01, 0x7F, 0x48, 0x48, 0x30 ,   // Џ 0xDA 218
    0x7F, 0x48, 0x30, 0x00, 0x7F ,   // џ 0xDB 219
    0x00, 0x7F, 0x48, 0x48, 0x30 ,   // № 0xDC 220
    0x22, 0x41, 0x49, 0x49, 0x3E ,   // Ё 0xDD 221
    0x7F, 0x08, 0x3E, 0x41, 0x3E ,   // ё 0xDE 222
...
void pix_to_buf(unsigned char x, unsigned char y, unsigned char draw)
{
	if(x>199||y>41) return;
	if(draw==1)
	{
...
void char_to_buf(unsigned char value, int column, int row)
{
	for(char i=0;i<5;i++)
	frame[(column*5)+i][row]=font[value-32][i];
}

void string_to_buf(unsigned char string_[])
{
	char row=0;
	char column=0;
	for(int i=0;i<strlen(string_);i++)
	{
		...
		if(i>=240) return;
		char_to_buf(string_[i],column,row);
		column++;
	}
}        
...
drawLine(199,0,0,41);
//drawCircle(99,20,19);
//string_to_buf("ƒобро пожаловать на сайт www.radiokot.ruѕривет всем любител€м газоразр€дных     девайсов! 01234567890јаЅб¬в√гƒде≈®Є∆ж«з»и…й кЋлћмЌнќоѕп–р—с“т”у‘ф’х÷ц„чЎшўщЏъџы№ьЁэёюя€AaBbCcDdEeFfGgHhIiJjKkLlMmNnOoPpQqRrSsTtUuVvWwXxYyZz!@#$%^&*()12356789123456ƒобро пожаловать на сайт www.radiokot.ruѕривет всем любител€м газоразр€дных     девайсов! 01234567890јаЅб¬в√гƒде≈®Є∆ж«з»и…й кЋлћмЌн");
//char_to_buf(152,0,1);
...
for(int r=0;r<6;r++)
{
 for(int j=0;j<7;j++)
	    {
		   for(int k=0;k<200;k=k+8)
		    {
			    for(int i=0;i<8;i++)
			    {
				    val|=((frame[i+k][r]>>j)&0x01)<<7-i;
			    }
			   		for(int b = 7; b >= 0; --b)
						{
						printf("%d", (val>>b)&1);
